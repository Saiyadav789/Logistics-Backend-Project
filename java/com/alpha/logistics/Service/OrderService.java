...
public class OrderService {
	...
	public ResponseEntity<ResponseStructure<Order>> placeuserOrder(OrderDto orderdto) {
		...
		Carrier carrier=carrierservice.find(105);
...
Order saveOrder=orderrepo.save(order);
	...
	String content="YOUR ORDER PLACED FROM "+order.getLoading().getAddress().getCity()+"TO"+order.getUnloading().getAddress().getCity()+"FOR THE COST OF"+order.getCost();
	mailserv.sendMail(orderdto.getEmail(), subject, content);
	...
	public void SendFinalMail(int orderid) {
	Optional<Order>	SendMail=orderrepo.findById(orderid);
	           Order order=SendMail.get();
	           String subject = "Order Updated";

	           String content = "Dear Customer,\n\n" +
	               "We’re pleased to inform you that your cargo order has been successfully updated.\n\n" +
	               "Here are the details of your order:\n" +
	               "--------------------------------------\n" +
	               "Loading City: " + order.getLoading().getAddress().getCity() + "\n" +
	               "Unloading City: " + order.getUnloading().getAddress().getCity() + "\n" +
	               "Cargo Name: " + order.getCargo().getName() + "\n" +
	               "Description: " + order.getCargo().getDescription() + "\n" +
	               "Total Cost: ₹" + order.getCost() + "\n" +
	               "Loading Schedule: " + order.getLoading().getDate() + " at " + order.getLoading().getTime() + "\n" +
	               "Unloading Schedule: " + order.getUnloading().getDate() + " at " + order.getUnloading().getTime() + "\n\n" +
	               "Thank you for choosing Alpha Logistics.\n" +
	               "We appreciate your trust in our services.\n\n" +
	               "Best regards,\n" +
	               "Alpha Logistics Team";
	           
	           mailserv.sendMail(order.getEmail(), subject, content);
